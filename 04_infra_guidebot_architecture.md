...
> 이 봇은 “답변 생성 모델”보다 “업무 라우팅 시스템”으로 설계해야 한다. Supervisor가 사용자의 의도를 절차 안내, 부서 문의, 신청 생성, 아키텍처 검토, 범위 외 문의로 먼저 나누고, Qwen3.5-120B는 복합 판단과 최종 답변에 집중시키는 구조가 가장 안정적이다.
...
3. Action: 답변만 필요한가, 문의/신청까지 필요한가?
...
| Redis 하나 신청해줘 | 신청 요청 | DBA | 신청서 초안 생성 |
| 운영 서버 방화벽 열어야 해요 | 절차/신청 | NE, Security, SE | 신청 지원 |
...
| Workflow Application Agent | 내부 결재/신청 시스템용 입력값 정리 |
...
## 7. 신청 지원은 상태머신으로 처리한다

“알려줘”와 “신청해줘”는 완전히 다르다. 신청 요청은 바로 생성하면 안 되고, 필수 입력값 확인과 사용자 승인 단계를 반드시 거쳐야 한다.
...
사용자 확인
  ↓
Workflow/Jira 신청 생성 또는 담당자 전달
  ↓
신청 번호 반환
```

실제 신청 생성 전에는 반드시 사용자에게 확인을 받아야 한다.

```text
봇: 아래 내용으로 신청서를 생성하려고 합니다. 진행할까요?
사용자: 진행해줘.
봇: 신청이 생성되었습니다. 신청 번호는 WF-12345입니다.
```
...
| action_type | guide, ask_department, create_request, draft_ticket |
...
9. 실제 신청 생성 전에 사용자 확인 단계가 있는가?
10. 부서 문의와 실제 신청을 혼동하지 않았는가?
11. 범위 외 문의를 억지로 답하지 않았는가?
12. 자동 판단이 어려운 경우 Human handoff를 제안했는가?
13. 개발자가 바로 행동할 수 있는 수준으로 답변했는가?
...
```json
{
  "intent": "procedure_guide|department_question|request_creation|architecture_analysis|out_of_scope",
  "is_in_scope": true,
  "domain": [],
  "required_requests": [],
...
> 인프라가이드봇은 단순 절차 안내 RAG가 아니라 문의 분류, 담당부서 라우팅, 신청 지원, 범위 외 문의 처리까지 포함하는 업무 포털형 에이전트로 설계해야 한다. Supervisor는 필수이며, Qwen 32B는 분류와 추출에, Qwen3.5-120B는 복합 판단과 최종 답변에, gpt-oss-120B는 검증과 위험 통제에 배치하는 것이 가장 안정적이다.
